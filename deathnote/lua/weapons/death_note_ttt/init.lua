...
-- this is here just for my simple sake (Bitch Code)
function DeathNote_TTT_Primary(ply, TarPly, TheDeathType)
	if not ply.DeathNoteUse then
		...
		if TarPly:Alive() then
			...
			timer.Simple(
				GetConVar("DeathNote_TTT_DeathTime"):GetInt(),
				function()
					-- Always Die
					if GetConVar("DeathNote_TTT_AlwaysDies"):GetBool() then
						...
						if table.HasValue(TTT_DN_Chance, rolled) then
							...
							if GetConVar("DeathNote_TTT_LoseDNOnFail"):GetBool() then
								...
								timer.Simple(
									GetConVar("DeathNote_TTT_DNLockOut"):GetInt(),
									function()
										if ply:Alive() then
